  <link rel="stylesheet" href="css/summernote/summernote.css">
  	<?php 
		...
		session_start();	
		if(isset($_GET["id"])){
			$fetch="select  * from tblblogs where id='".$_GET["id"]."'";
...
if(isset($_POST["submit"])){
	...
	if($name!="" && ($type="image/jpeg") || $size<=614400)
	{
		...
		{
		$sql="INSERT INTO `tblblogs`(`title`,`type`, `description`, `image`,`storeDate`) VALUES ('".$_POST['name']."','".$_POST['type']."','".$_POST["description"]."','".$name."','".$currentDate."')";
...
if(isset($_POST["update"])){
		...
		if($name=="")
			...
			if($name!="" && ($type="image/jpeg") || $size<=614400)
			{
				...
				$sql="UPDATE `tblblogs` SET `title`='".$_POST['name']."',`description`='".$_POST["description"]."',`type`='".$_POST["type"]."',`image`='".$name."',`storeDate`='".$currentDate."' WHERE `id`='".$_GET["id"]."'";
                                    ...
                                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                        <ul class="breadcome-menu">
                                           <li><a href="dashboard.php">Dashboard</a> <span class="bread-slash">/</span>
                                            ...
                                            <li><span class="bread-blod">Update Blogs</span>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
